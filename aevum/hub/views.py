...
from .forms import CodeSnippetForm, EventForm, FocusSessionForm, NoteForm, ProfileForm, RegisterForm, TagForm, TaskForm
...
def register_view(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        login(request, user)
        messages.success(request, 'Welcome to Aevum. Your student workspace is ready.')
        return redirect('dashboard')
...
def _streak_days(user):
    ...
    days = done_task_days | focus_days
    if not days:
        return 0
...
def dashboard(request):
    ...
    events = request.user.events.order_by('start_date')[:5]
    stats = {
        'notes': notes.count(),
        ...
        'events': request.user.events.count(),
    ...
    for i in range(28):
        ...
        count = request.user.focus_sessions.filter(completed_at__date=d).count() + request.user.tasks.filter(status='done', updated_at__date=d).count()
...
def profile_settings(request):
    ...
    if request.method == 'POST' and form.is_valid():
        form.save()
        messages.success(request, 'Profile and GitHub settings updated successfully.')
...
def search_view(request):
    ...
    return render(request, 'search.html', {'query': query, 'notes': notes, 'tasks': tasks, 'events': events, 'snippets': snippets})
...
def focus_timer(request):
    form = FocusSessionForm(initial={'minutes': 25})
...
def _create_or_get_share(user, target_type, note=None, snippet=None):
    obj, _ = SharedLink.objects.get_or_create(user=user, target_type=target_type, note=note, snippet=snippet)
    return obj
