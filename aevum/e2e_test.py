...
from hub.models import CodeSnippet, Event, FocusSession, Note, SharedLink, Tag, Task
...
def main():
    ...
    ok('register redirects to dashboard', response.status_code == 200 and b'Welcome back, student1' in response.content)
    ...
    ok('create note', response.status_code == 200 and b'Calculus Chapter 5' in response.content and b'Download file' in response.content)
    ...
    response = client.post('/focus/log/', {'minutes': 25, 'note': 'Study session'}, follow=True)
    ok('focus session logged', response.status_code == 200 and FocusSession.objects.filter(note='Study session').exists())

    response = client.get('/search/?q=math')
    ok('search finds note or event', b'Calculus Chapter 5' in response.content or b'Math Midterm' in response.content)

    response = client.get('/dashboard/')
    ok('dashboard renders', response.status_code == 200 and b'Consistency Heatmap' in response.content and b'Academic Activity Overview' in response.content)

    response = client.get('/share/profile/')
    ok('share profile page ready', response.status_code == 200 and b'Public Share Link Ready' in response.content)
    ...
    ok('api stats endpoint', response.status_code == 200 and response.json()['notes'] == 1 and response.json()['snippets'] == 1)
